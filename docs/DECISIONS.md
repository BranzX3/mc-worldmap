...
## กฎที่กดผิวน้ำหลายตัวต้องวนจนลู่เข้า ไม่ใช่ทำอย่างละครั้ง

`shape_waterway_patch` มีกฎสองตัวที่แก้ผิวน้ำคนละแกน: `limit_masked_steps`
(ขั้นตามแนวลำน้ำ) กับ `flatten_cross_sections` (ผิวน้ำเท่ากันทั้งหน้าตัด)
เดิมทำอย่างละครั้งตามลำดับ ซึ่งไม่พอ — การยุบหน้าตัดกด cell ลงหาค่าต่ำสุด
เพื่อนบ้าน *ตามแนวลำน้ำ* ที่ยังสูงอยู่จึงกลายเป็นขั้นใหม่ที่ envelope ไม่เคยเห็น
และไม่ผ่าน `supported` จึงไม่มีม่านน้ำมาปิด = ช่องว่างกลางสายน้ำ

วัดบนผังลำน้ำคดเคี้ยวตัดหน้าผา 15 บล็อก: ทำอย่างละครั้งเหลือช่องว่าง 1 จุด,
วนจนลู่เข้าเหลือ 0 (`SURFACE_FIXPOINT_PASSES`, ตั้ง 1 เพื่อจำลองของเดิมได้)
วนแล้วหยุดจริงเพราะทั้งสองกฎ *กดลงอย่างเดียว* ยกเว้น cell ที่ pin ไว้ซึ่งมีค่า
ตายตัว  `test_flattening_does_not_reopen_a_gap_the_envelope_already_closed`
คุมไว้ และ parity test ของ global/patch ยังผ่าน (การวนไม่ได้ทำให้ envelope
แพร่ไกลเกิน halo)

### การยุบหน้าตัด: pin ต้องแผ่ไปทั้ง run

`flatten_cross_sections` ใช้ค่าต่ำสุดของ run ยกเว้น run ที่แตะปากทะเลสาบซึ่ง
ใช้ระดับทะเลสาบ (ยกขึ้นได้) ถ้า pin ไม่แผ่ตามไปด้วย อีกแกนหนึ่งจะเห็น cell
ที่เพิ่งถูกยกเป็น cell ธรรมดา แล้วกดกลับด้วยค่าต่ำสุดของตัวเอง สลับกันไปจนหมด
จำนวนรอบ — ฟังก์ชันจึง **คืน run ที่ยังไม่ราบ** ทั้งที่นั่นคือหน้าที่เดียวของมัน
พอ pin แผ่ตามไปด้วย เซตที่ถูก pin โตทางเดียว ส่วนที่เหลือมีแต่ถูกกด การวนจึงหยุด

## บั๊กที่รอดสายตาได้ ล้วนให้ค่าที่ "ดูสมเหตุสมผล"

ทบทวนบั๊กทุกตัวที่เกิดในไฟล์ `golden_patches.py` ระหว่างเขียนมันเอง แล้วพบว่า
แบ่งได้สองกลุ่มชัดเจน

| เจอเร็ว (ภายในนาที) | รอดมาได้หลายรอบ |
|---|---|
| กล้องไปยืนบนสันเขาแล้วเห็นแต่ฟ้า | `water_column_max = 1` **ทุก cell ทั้งแผนที่** |
| หน้าตัดไม่มีน้ำสีฟ้าเลย | `np.roll` พันขอบ ทำให้ผนังมาจากอีกฟากของกรอบ |
| `--only` แล้วเหลือจุดเดียวใน sheet | cache ไม่มีลายนิ้วมือ (`cached` ทุกจุดดูปกติมาก) |

ตัวที่เจอเร็วคือตัวที่ผลลัพธ์ **ขัดกับสิ่งที่คาดไว้อย่างเห็นได้ชัด** ส่วนตัวที่
รอดมาได้คือตัวที่ให้ค่าที่ดูปกติจนไม่มีอะไรสะดุด — ไม่ใช่เพราะโค้ดซับซ้อนกว่า

**บทสรุปที่ใช้ได้จริง: อย่าหวังว่าจะ "ระวังมากขึ้น" แล้วไม่พลาด ให้เขียนโค้ดที่
ขัดแย้งกับตัวเองได้เมื่อผิด** สามด่านที่ใช้อยู่ตอนนี้ ทุกด่านทำงานทุกครั้งโดย
ไม่ขึ้นกับว่าคนเขียนตั้งใจแค่ไหนในรอบนั้น

1. **`audit_metrics`** — ตรวจว่าตัวเลขขัดกับข้อเท็จจริงที่รู้อยู่แล้วไหม (สัดส่วน
   ต้องอยู่ 0..1, นับ cell ต้องไม่เกินจำนวน cell, `water_column_max` ต้องตรงกับ
   `depth` ที่คำนวณคนละทาง) ไม่ได้ตรวจว่า "ถูก" — ตรวจว่า "เป็นไปได้"
2. **`selftest`** — เทียบกับสูตร naive ที่เขียนคนละแบบ + ผังที่รู้คำตอบอยู่แล้ว
   + ยัดค่าที่เป็นไปไม่ได้เพื่อดูว่าด่าน 1 ยังไม่ตายไปเอง
3. **`flag_degenerate`** — เตือนเมื่อตัวเลขตัวหนึ่งเท่ากันหมดทั้ง 9 จุด ทั้งที่
   ทั้ง 9 จุดถูกเลือกมาให้ต่างกัน  ด่านนี้ดัก `water_column_max = 1` ได้ตรง ๆ

ยืนยันด้วย mutation test แล้วว่าใส่บั๊กเดิมสองตัวกลับไป `selftest` จับได้ทั้งคู่
(exit 1) ไม่ใช่แค่ "มีเทสต์อยู่"

**กฎที่ตามมา**: ฟังก์ชันที่มองรอบตัว (หน้าต่าง/เพื่อนบ้าน/EDT) ต้องมีเทสต์ที่
**ขอบอาร์เรย์** เสมอ และ metric ใหม่ทุกตัวต้องมีอย่างน้อยหนึ่งการตรวจไขว้กับ
ข้อมูลอีกชุดที่คำนวณคนละทาง ไม่งั้นมันจะกลายเป็นตัวเลขที่ไม่มีใครเถียงได้

## compare ต้องมีธรณีประตูของเสียง ไม่งั้นใช้เป็นด่านไม่ได้

`golden_patches.py compare` เคยรายงาน "ถอยหลัง 22 ตัว" จากการขยับระดับ 0.1-0.3
percentage point ซึ่งเป็นผลข้างเคียงปกติของการเพิ่มรอบการวน ของจริงที่ต้องเห็น
(ช่องว่างกลางสายน้ำ 8 -> 1) จมหายไปในรายการ

ตอนนี้สัดส่วนต้องขยับ >= 1 percentage point และจำนวนนับต้องขยับ >= 5% (อย่างน้อย
2) จึงนับว่าถอยหลัง **ยกเว้นตัวที่ต้องเป็นศูนย์ — ขยับ 1 ก็คือถอยหลัง**

หลังใส่ธรณีประตู รอบเดียวกันเหลือ 1 รายการจริง (`bank_wall_cells` 295 -> 312 ที่
patch ซึ่งเต็มไปด้วยหน้าผาจริง) ซึ่งอ่านแล้วตัดสินใจได้ใน 5 วินาที

## halo ต้องโตตามระยะที่กฎแพร่จริง — วัดใหม่ทุกครั้งที่เปลี่ยนกฎ

DECISIONS เคยบันทึกว่า "halo 8 พอแล้ว — วัดมาแล้ว อย่าเดาเพิ่ม" **ข้อนั้นหมดอายุ
แล้ว** เพราะกฎเปลี่ยน: ตอนนี้ envelope กับการยุบหน้าตัดวนกันได้ถึง 16 รอบ ผิวน้ำ
จึงกลายเป็นวัตถุที่ผูกกันเป็นระยะไกล ไม่ใช่กฎท้องถิ่นอีกต่อไป

วัดจาก `--global` เต็มแผนที่ (10,000², tile 512):

| halo | ขั้น >=2 ที่ขอบ tile | ภายใน tile | เวลารัน |
|---|---|---|---|
| 24 (เดิม) | 32.7% | 2.5% | ~17 นาที |
| 64 | 18.0% | 2.5% | ~25 นาที |
| **128 (ค่าใหม่)** | **9.1%** | 2.5% | ~50 นาที |

และตรวจด้วย `audit_global.py` (สุ่ม 30 หน้าต่าง ครึ่งหนึ่งคร่อมรอยต่อ tile):

| halo | หน้าตัดไม่ราบ | ช่องว่าง | ตลิ่งลอย |
|---|---|---|---|
| 24 | 34 | 7 | 3 |
| 64 | 9 | **0** | 1 |
| **128** | **1** | **0** | **0** |

ตั้ง `GLOBAL_HALO = 128` เป็นค่า default แล้ว และปรับเกณฑ์เตือนใน seam report จาก
1.5 เท่าเป็น 4 เท่าของอัตราภายใน tile — ที่ 128 อัตราส่วนคือ 3.7 เท่าและ audit
บอกว่า invariant เป็นศูนย์แล้ว ถ้าปล่อยเกณฑ์เดิมไว้มันจะเตือนตลอดกาลจนคนเลิกอ่าน
(ราคาของการเพิ่มต่อ: ทุกการเพิ่มเท่าตัวลดขั้นที่ขอบได้ครึ่งเดียวแต่เวลาโตเท่าตัว)

(เส้นทาง `--patch` ได้ 0/0/0 ทั้งหมด — ตัวเลขที่เหลือคือราคาของการแบ่ง tile ล้วน ๆ)

**บทเรียนที่ต้องไม่ลืมอีก**: ทุกครั้งที่เพิ่มกฎที่ *วนจนลู่เข้า* ระยะแพร่จะไม่ใช่
ค่าคงที่อีกต่อไป ต้องรัน `--global` แล้วอ่าน `seam_step2plus_share` เทียบกับ
`interior_step2plus_share` ในไฟล์ manifest ทุกครั้ง ไม่ใช่เชื่อค่า halo เดิม
